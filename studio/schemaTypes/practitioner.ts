...
export default defineType({
  ...
  fields: [
    ...
    defineField({name: 'slug', title: 'כתובת (slug)', type: 'slug', group: 'main', options: {source: 'name', maxLength: 96}, validation: (r) => r.required()}),
    ...
    defineField({
      name: 'videoUrl', title: 'קישור וידאו (יוטיוב/וימאו)', type: 'url', group: 'main',
      description: 'מופיע בראש עמוד הדולה — מנוע ההמרה',
    }),
    defineField({name: 'tier', title: 'רמה', type: 'string', group: 'main',
      options: {list: [{title: 'מובחרת — עמוד מלא', value: 'premium'}, {title: 'אינדקס — רישום קצר', value: 'index'}], layout: 'radio'},
      initialValue: 'premium', validation: (r) => r.required()}),

    // appears in which engine
    defineField({name: 'isDoula', title: 'מופיעה בחיפוש דולות', type: 'boolean', group: 'main', initialValue: true}),
    defineField({name: 'isProfessional', title: 'מופיעה בחיפוש נשות מקצוע', type: 'boolean', group: 'main', initialValue: false}),

    // --- doula search facets ---
    defineField({name: 'hospitals', title: 'בתי חולים', type: 'array', group: 'doula',
      of: [{type: 'reference', to: [{type: 'hospital'}]}]}),
    defineField({name: 'supportStyle', title: 'סגנון ליווי', type: 'array', group: 'doula',
      of: [{type: 'string'}], options: {list: SUPPORT_STYLES}}),
    defineField({name: 'budget', title: 'תקציב', type: 'string', group: 'doula',
      options: {list: BUDGET_BANDS}}),
    defineField({name: 'availableAround', title: 'זמינות סביב תאריך', type: 'date', group: 'doula',
      description: 'לסינון לפי תאריך לידה משוער — מנגנון לאישור מול קרן'}),

    // --- professional search facets ---
    defineField({name: 'fields', title: 'תחומי טיפול', type: 'array', group: 'pro',
      of: [{type: 'reference', to: [{type: 'field'}]}], description: 'הנקה / עיסוי / רפלקסולוגיה …'}),
    defineField({name: 'regions', title: 'אזורים (מיקום)', type: 'array', group: 'pro',
      of: [{type: 'reference', to: [{type: 'region'}]}]}),
    defineField({name: 'languages', title: 'שפות', type: 'array', group: 'pro',
      of: [{type: 'string'}], options: {list: ['עברית', 'אנגלית', 'ספרדית', 'רוסית', 'ערבית', 'צרפתית']}}),

    // --- contact ---
    defineField({name: 'phone', title: 'טלפון', type: 'string', group: 'contact'}),
    defineField({name: 'whatsapp', title: 'וואטסאפ', type: 'string', group: 'contact', description: 'מספר לקישור click-to-chat'}),
    ...
    defineField({name: 'services', title: 'מה כולל הליווי', type: 'array', group: 'content', of: [{type: 'block'}]}),
    ...
    defineField({name: 'testimonials', title: 'המלצות', type: 'array', group: 'content',
    ...
    defineField({name: 'faq', title: 'שאלות ותשובות', type: 'array', group: 'content',
      of: [{type: 'object', fields: [
